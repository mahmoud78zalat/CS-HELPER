...
-- Includes error handling for existing variables - Uses template_variables table with example column

-- Insert Customer Info Panel Variables
INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'customer_name', 'Customer Name', 'Customer Information', 'John Smith', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'customer_name');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'customer_phone', 'Customer Phone Number', 'Customer Information', '+971 50 123 4567', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'customer_phone');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'customer_country', 'Customer Country', 'Customer Information', '🇦🇪 United Arab Emirates', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'customer_country');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'gender', 'Customer Gender', 'Customer Information', 'Male', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'gender');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'language', 'Customer Language Preference (en/ar)', 'Customer Information', 'en', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'language');

-- Insert Agent Information Variables
INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'agentarabicfirstname', 'Agent Arabic First Name', 'Agent Information', 'أحمد', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'agentarabicfirstname');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'agentarabiclastname', 'Agent Arabic Last Name', 'Agent Information', 'محمد', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'agentarabiclastname');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'agentarabicname', 'Agent Arabic Full Name (First + Last)', 'Agent Information', 'أحمد محمد', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'agentarabicname');

-- Insert Additional Info Panel Variables
INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'itemname', 'Product/Item Name', 'Order Information', 'Nike Air Max Shoes', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'itemname');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'deliverydate', 'Delivery Date', 'Order Information', '2025-08-05', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'deliverydate');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'waitingtime', 'Waiting Time/Processing Time', 'Order Information', '2-3 business days', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'waitingtime');

-- Insert Live Reply Template Language Variables
INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'customerfirstname', 'Customer First Name (extracted from full name)', 'Customer Information', 'John', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'customerfirstname');

INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'agentfirstname', 'Agent First Name (English)', 'Agent Information', 'Ahmed', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM template_variables WHERE name = 'agentfirstname');

-- Insert User ID variable for order conversion
INSERT INTO template_variables (name, description, category, example, created_at, updated_at)
SELECT 'userid', 'User ID for order conversion', 'System Information', 'USR123456', NOW(), NOW()
