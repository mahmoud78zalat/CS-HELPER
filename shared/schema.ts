...
export const users = pgTable("users", {
  ...
  arabicFirstName: varchar("arabic_first_name"),
  arabicLastName: varchar("arabic_last_name"),
  profileImageUrl: varchar("profile_image_url"),
  role: userRoleEnum("role").default("agent").notNull(),
  status: userStatusEnum("status").default("active").notNull(),
  isOnline: boolean("is_online").default(false).notNull(),
  isFirstTimeUser: boolean("is_first_time_user").default(true).notNull(),
...
export const storeEmails = pgTable("store_emails", {
  ...
  storePhone: varchar("store_phone"), // Made optional - phone is not required
  isActive: boolean("is_active").default(true).notNull(),
  orderIndex: integer("order_index").default(0).notNull(),
  createdBy: uuid("created_by").references(() => users.id),
  createdAt: timestamp("created_at").defaultNow(),
